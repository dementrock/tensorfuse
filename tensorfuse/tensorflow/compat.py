...
def _tf_shape(self):
    ...
    try:
        ...
        none_dims = [idx for idx, size in enumerate(shape_template) if size is None or hasattr(size,
                                                                                               "value") and size.value is None]
        if len(none_dims) > 0:
            ...
            return [x if shape_template[idx] is not None and shape_template[idx].value is not None else tf.shape(self)[
                idx] for idx, x in
                    enumerate(nominal_shape)]
        else:
...
def _mk_tf_getitem(old_getitem):
    ...
    def getitem(self, slices):
        try:
            ...
            rev_dims = [idx for idx, x in enumerate(rest_slices) if
                        isinstance(x, slice) and x.start is None and x.stop is None and x.step == -1]
            ...
            if isinstance(target, tf.Tensor):
                ...
                import ipdb;
                ipdb.set_trace()
            if any(isinstance(x, tf.Tensor) for x in rest_slices):
                # in this case the slice is of the form x[tensor, :, :]
                # apply gather in this case
                if all(x == slice(None) for x in rest_slices[1:]) and len(rest_slices[0].shape) == 0:
                    return tf.gather(target, rest_slices[0])
                else:
                    raise NotImplementedError
                import ipdb;
                ipdb.set_trace()
            elif len(none_dims) == 0:
